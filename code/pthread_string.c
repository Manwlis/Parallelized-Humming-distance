/*
 ...
 Description :	Task is set as: Compare one string from group A with another from group B.
 				For each different character add 1 to the diffs[i][j].
...
void *compute_hamming(void *thread_info){
	...
	//	Each threads calculates it's iterations.
	int my_iterations = (int) tinfo->lines * tinfo->rows / tinfo->NumThreads;
	int start  = my_iterations * tinfo->tid;
	int finish = start + my_iterations;

	/*	In case the cells/threads division is not perfect, a few iterations will be leftover.
		The last thread will take the extra work.												*/
	if(tinfo->tid == tinfo->NumThreads - 1){
		finish += (tinfo->lines * tinfo->rows) % tinfo->NumThreads;
	}

	/*	Count allows the thread to work only on the pairs that is assigned to. 
		Starts from the line the thread needs to work first and make it 
		disregard the first pairs which are not assigned( If such pairs exists) to it. */
	int startingLine = tinfo->tid * tinfo->lines / tinfo->NumThreads;
	int count = startingLine * tinfo->rows;

	for( int i = startingLine; i < tinfo->lines; i++){
		for(int j = 0; j < tinfo->rows; j++){
			
			//	Disregard unassigned pairs.
		...
		//	When the thread finish the assigned pairs, count breaks it out of the loop.
		if (count == finish)
			break;	
	}
	...
	//	Exit. Master thread needs to return to continue the program.
	if(tinfo->tid != 0){
		pthread_exit(NULL);
	}
	return NULL;
