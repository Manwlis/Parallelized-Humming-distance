/*
 ...
 Description :	Task is set as: Compare one char from a string from group A with another one from a string from group B.
For each different character add 1 to the diffs[i][j]. Then each thread has to add that to the total diff.
 ============================================================================
 */
...
void *compute_hamming(void *thread_info){
	...
	for(int i = 0; i < tinfo->lines; i++){
		for(int j = 0; j < tinfo->rows; j++){
			for(int pshfio = tinfo->tid; pshfio < tinfo->lenght; pshfio += tinfo->NumThreads){

				if(tinfo->StringsA[i][pshfio] != tinfo->StringsB[j][pshfio]){

					__atomic_fetch_add(&(tinfo->df[i][j]), 1, __ATOMIC_SEQ_CST);
					threadSum++;
				}
			}		
		}
